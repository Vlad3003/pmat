...
class Database(metaclass=SingletonMeta):
    ...
    def insert(self, table_name, data, *, sep: Literal[' ', ','] = ' ') -> None:
    ...
    def join(
            ...
            join_attrs: list[tuple[str, str]]
    ) -> list[_RT]:
        """
        Объединяет несколько таблиц по указанным аттрибутам.

        :param tables: Кортеж имен таблиц.
        :param join_attrs: Список кортежей, где каждый кортеж представляет пары 'таблица.атрибут' для соединения.
        :return: Результат объединения таблиц, если удалось объединить таблицы по указанным атрибутам, иначе пустой список.
        """
        if len(tables) < 2:
            raise ValueError("At least two tables are required to perform a join.")
        ...
        if len(join_attrs) != len(tables) - 1:
            raise ValueError("Number of join attributes must be one less than the number of tables.")

        for attrs in join_attrs:
            for attr in attrs:
                if not attr.count("."):
                    raise ValueError(
                        f"Join attribute '{attr}' must have the following format 'table_name.table_attribute'."
                    )
                ...
                if _attr not in _table.ATTRS:
                    raise ValueError(f"'{_attr}' is not an attribute of table '{_table_name}'.")
            ...
            for row1 in result:
                for row2 in table.data:
                    if row1[join_attr1] == row2[join_attr2]:
                        new_row2 = {f"{tables[i]}.{key}": value for key, value in row2.items()}
    ...
    def aggregate(
        ...
        :param operation: Операция агрегации ('SUM', 'AVG', 'COUNT', 'MIN', 'MAX')
        ...
        if group_by and group_by not in table.ATTRS:
            raise ValueError(f"Column '{group_by}' does not exist in table '{table_name}'.")
        ...
        if operation in ("AVG", "SUM") and not all(
                ...
                for row in table.data for sym in row[column]
        ):
            raise TypeError(
        ...
        if not group_by:
            values = [
                float(row[column]) if row[column].count(".") == 1
                else int(row[column]) if all(sym.isdigit() for sym in row[column])
                else row[column]
            ...
            for row in table.data:
                ...
                elif all(sym.isdigit() for sym in row[column]):
            ...
            return [
                {f"{group_by}": key, f"{operation}({column})": str(aggregate_func(value))} 
    ...
    def _check_unique(self, new_row: dict[str, str]) -> None:
        ...
        for unique_attr in self.UNIQUE_ATTRS:
            if isinstance(unique_attr, str):
                if any(row[unique_attr] == new_row[unique_attr] for row in self.data):
                    raise ValueError(
                        f"Value '{new_row[unique_attr]}' already exists in "
                        f"column '{unique_attr}' of the '{self.__class__.__name__}', "
                        f"which must be unique."
                    )
            else:
                if any(all(row[attr] == new_row[attr] for attr in unique_attr) for row in self.data):
                    values = ', '.join(f"{attr}={new_row[attr]}" for attr in unique_attr)
                    raise ValueError(
                        f"Values '{values}' already exists in "
                        f"columns {unique_attr} of the '{self.__class__.__name__}', "
                        f"which must be unique."
                    )

    def _validate_data(self, new_row: dict[str, str]) -> None:
        """
        Проверяет целостность и корректность данных новой строки перед добавлением в таблицу.

        Метод выполняет базовую проверку новой строки `new_row`, чтобы убедиться,
        что количество переданных данных совпадает с количеством атрибутов таблицы.
        Если количество значений не совпадает, метод выбрасывает исключение `ValueError`.
        """
        if len(new_row) != len(self.ATTRS):
            raise ValueError(
                f"The number of values in the new row '{len(new_row)}' "
                f"does not match the number of '{self.__class__.__name__}' attributes '{len(self.ATTRS)}'."
            )
...
class EmployeeTable(Table):
    ...
    def select(self, start_id: int, end_id: int) -> list[dict[str, str]]:
        return [row for row in self.data if start_id <= int(row['id']) <= end_id]
...
class DepartmentTable(Table):
    ...
    def select(self, department_name) -> list[dict[str, str]]:
        return [row for row in self.data if row['department_name'] == department_name]
...
class ProjectTable(Table):
    ...
    def _validate_data(self, new_row: dict[str, str]) -> None:
        """
        Проверяет корректность данных новой строки перед добавлением в таблицу, включая значения и диапазон дат.

        Если значение 'start_date' позже 'end_date', а также, если одна из этих дат имеет некорректное значение,
...
class EmployeeProjectTable(Table):
    ...
    def select(
        ...
        if employee_id and project_id:
            result = [
                row for row in self.data if int(row['employee_id']) == employee_id
            ...
            result = [
                row for row in self.data if int(row['project_id']) == project_id
            ]
        else:
            result = []
